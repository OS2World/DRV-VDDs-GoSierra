...
BOOL MINSTALL_FilterControlFileData (VOID) {
   ...
   while (CurNo<MCF_GroupCount) {
      ...
      if (CurGroupPtr->ID==0) {
         ...
         switch (CurGroupPtr->AutoSelect) {
          ...
          case MINSTGRP_Select_Version:
            ...
            //  choose anyway.
            if (strlen(CurGroupPtr->VersionInstalled)>0)
               if (CurGroupPtr->VersionCode>CurGroupPtr->VersionInstalledCode)
                  CurGroupPtr->Flags |= MINSTGRP_Flags_Selected;
            break;
      ...
      //  one was found. Also it will check for the DLL to be listed in file-
...
###
      ...
      if (CRC32) {
         if (!(CurFilePtr = MINSTALL_SearchFileCRC32(CRC32))) {
            ...
            MINSTALL_TrappedError (MINSTMSG_UnlistedScript);
            return FALSE;                   // File not found, so error
          }
...
BOOL MINSTALL_GeneratePublicGroup (void) {
   ...
   if (MINSTALL_PublicGroupCount) {
      ...
      CurGroupNo = 0; CurGroup = MCF_GroupArrayPtr;
      while (CurGroupNo<MCF_GroupCount) {
         if (!(CurGroup->Flags & MINSTGRP_Flags_DontListPublic)) {
            CurPublicGroup->ID = CurGroup->ID;
            if (CurGroup->Flags & MINSTGRP_Flags_Selected)
               CurPublicGroup->Selected = TRUE;
            if (CurGroup->Flags & MINSTGRP_Flags_SelectionForced)
               CurPublicGroup->SelectionForced = TRUE;
            strcpy (CurPublicGroup->Name, CurGroup->Name);
            strcpy (CurPublicGroup->Version, CurGroup->Version);
            strcpy (CurPublicGroup->VersionInstalled, CurGroup->VersionInstalled);
            CurPublicGroup->SpaceNeeded = CurGroup->SpaceNeeded;
            if (CurGroup->GeninPtr) {
               CurPublicGroup->MaxCardCount = CurGroup->GeninPtr->MaxCardCount;
               CurPublicGroup->PromptsCount = CurGroup->GeninPtr->PromptsCount;
               for (CurPromptNo=0; CurPromptNo<MINSTMAX_GENINPROMPTS; CurPromptNo++) {
                  CurPublicGroup->PromptTitlePtr[CurPromptNo] = CurGroup->GeninPtr->PromptTitlePtr[CurPromptNo];
                  CurPublicGroup->PromptChoiceCount[CurPromptNo] = CurGroup->GeninPtr->PromptChoiceCount[CurPromptNo];
                  CurPublicGroup->PromptChoiceDefault[CurPromptNo] = CurGroup->GeninPtr->PromptChoiceDefault[CurPromptNo];
                  CurPublicGroup->PromptChoiceStrings[CurPromptNo] = CurGroup->GeninPtr->PromptChoiceStrings[CurPromptNo];
                }
             }
            CurPublicGroup++;
          }
