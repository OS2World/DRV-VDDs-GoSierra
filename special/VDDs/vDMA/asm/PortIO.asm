...
;      mov     bptr [CONST_Debug_PortOut+6], dl
;      mov     bptr [CONST_Debug_PortOut+7], al
;      push    8
...
VDMA_InOnDMA                    Proc Near   Uses ebx ecx edx esi edi
   ...
   pop     edx
;   MPush   <eax,ebx,ecx,edx,esi,edi>
;      mov     bptr [CONST_Debug_PortIn+6], dl
;      mov     bptr [CONST_Debug_PortIn+7], al
;      push    8
...
;   jnz     VDMAPIA_2ndByte
;   call    VDMA_UpdateTransferLeft       ; Update current DMA Channel position
...
VDMA_Ports_InCurLength          Proc Near
   ...
   call    VDMA_UpdateTransferLeft       ; Update current DMA Channel position
   mov     al, bptr [esi+VDMAslotStruc.TempDMAleft]
   xor     [esi+VDMAslotStruc.FlipFlop], VDMAslot_FlipFlop_Length
   ret
  VDMAPICP_2ndByte:
   mov     al, bptr [esi+VDMAslotStruc.TempDMAleft+1]
   xor     [esi+VDMAslotStruc.FlipFlop], VDMAslot_FlipFlop_Length
   ret
...
VDMA_Ports_OutLength            Proc Near
   test    [esi+VDMAslotStruc.FlipFlop], VDMAslot_FlipFlop_Length
   jnz     VDMAPOL_2ndByte
   mov     bptr [esi+VDMAslotStruc.TempDMASize], al
   xor     [esi+VDMAslotStruc.FlipFlop], VDMAslot_FlipFlop_Length
   ret
  VDMAPOL_2ndByte:
   mov     bptr [esi+VDMAslotStruc.TempDMASize+1], al
   movzx   eax, wptr [esi+VDMAslotStruc.TempDMASize]
   inc     eax                           ; 0 means 1 in DMA size
   cmp     [esi+VDMAslotStruc.DMAno], 4
   jb      Is8BitChannel
   shl     eax, 1                        ; WORD instead of BYTE size
  Is8BitChannel:
   mov     [esi+VDMAslotStruc.TransferSize], eax
   xor     [esi+VDMAslotStruc.FlipFlop], VDMAslot_FlipFlop_Length
   ret
...
VDMA_Ports_InStatus             Proc Near
   push    esi
     ...
     ChannelLoop:
         push    ecx
            call    VDMA_UpdateTransferLeft ; Update DMA Channel position
         pop     ecx
         add     esi, VDMAslot_Length
      dec     ecx
      jnz     ChannelLoop
